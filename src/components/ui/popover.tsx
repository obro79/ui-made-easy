...
export function PopoverContent({ side = "bottom", ...props }: HTMLAttributes<HTMLDivElement> & { side?: "top" | "right" | "bottom" | "left" }) { const { open, contentId } = usePopover(); const ref = useRef<HTMLDivElement>(null); useEffect(() => { if (open) requestAnimationFrame(() => ref.current?.querySelector<HTMLElement>('input, button, [href], [tabindex]:not([tabindex="-1"])')?.focus()) }, [open]); if (!open) return null; return <div ref={ref} id={contentId} role="dialog" aria-label={props["aria-label"] ?? "Popover"} tabIndex={-1} {...props} className={cn("ui-popover-content", `ui-popover-${side}`, props.className)} /> }
